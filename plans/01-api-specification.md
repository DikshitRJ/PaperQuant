# Plan 1: API Specification & Documentation

> **Phase**: 1 of 4 (Must complete BEFORE Plans 2 & 3)  
> **Estimated Effort**: ~4 hours  
> **Blocking**: Plans 2 and 3 depend on this spec as the single source of truth  
> **Output**: `/docs/api-spec.md` — the formal contract both backend and frontend agents will implement against

---

## 1. Goal

Create a complete, unambiguous API specification document that defines every endpoint, data schema, WebSocket event, and error contract between the Python backend (FastAPI sidecar) and the React frontend. This replaces the current `pywebview` bridge with a standard HTTP + WebSocket architecture suitable for Tauri packaging.

---

## 2. Architectural Shift: pywebview → FastAPI HTTP API

### Current Architecture (Broken)
```
React Frontend ←→ window.pywebview.api.* ←→ Python API class (in-process)
                                              ↓
                                    ProcessManager (Popen)
                                    ↓               ↓
                              Price_adapter    Trade_adapter
```
...
└──────────────┘                  │  │  Trade_adapter (subprocess)   │   │
                                  ...
                                  │  SQLite (candle history)             │
                                  └──────────────────────────────────────┘
```

### Why This Change?
1. **pywebview is dead weight in Tauri** — Tauri has its own WebView; pywebview is redundant
2. **HTTP API is testable** — curl, Postman, integration tests can hit endpoints
3. **Clean separation** — frontend and backend can be developed/tested independently
4. **WebSocket for real-time** — replaces the crude `evaluate_js()` polling hack
...
> [!IMPORTANT]
> The backend must compute `invested`, `current`, `pnl`, and format them as display strings. The current backend only stores `{qty, avg_price}` in DiskCache — it must be enriched with live price data.

---

#### `GET /api/positions/history`
Returns historical trade log from `order_history.csv`.
...
## 7. File Structure for API Documentation

```
docs/
├── api-spec.md           ← This document (rendered from this plan)
├── schemas/
│   ├── position.json     ← JSON Schema for Position
│   ├── algorithm.json    ← JSON Schema for Algorithm
│   ├── settings.json     ← JSON Schema for Settings
│   └── events.json       ← JSON Schema for WebSocket events
└── examples/
    ├── start-session.sh  ← curl example
    ├── get-positions.sh  ← curl example
    └── websocket.py      ← Python WebSocket client example
```
...
This table maps each API endpoint to the existing backend code that needs to be wrapped/adapted:

| Endpoint | Existing Backend Code | Adaptation Needed |
|---|---|---|
| `GET /api/health` | None | New: check subprocess status via `ProcessManager` |
| `POST /api/session/start` | `API.start_adapters()` + file I/O | Write `stocklist.json`, launch adapters + strategy |
| `POST /api/session/stop` | `API.stop_session()` | Wrap existing `ProcessManager.terminate()` |
| `POST /api/session/reset` | `API.reset_session()` | Wrap existing reset logic |
| `GET /api/positions` | `API.get_positions()` + DiskCache | Enrich with live price, compute P&L, format strings |
| `GET /api/positions/history` | `order_history.csv` | Parse CSV, return JSON |
| `GET /api/stats` | None | New: aggregate from positions + session timer |
| `GET /api/stats/chart` | None | New: track P&L snapshots over time |
| `GET /api/algorithms` | None | New: read from `algorithms/` directory + SQLite metadata |
| `POST /api/algorithms` | None | New: save file + register metadata |
| `DELETE /api/algorithms/{id}` | None | New: delete file + metadata |
| `POST /api/algorithms/{id}/run` | None | New: spawn strategy subprocess via Handler.py |
| `GET /api/settings` | None | New: read from DiskCache/JSON config |
| `PUT /api/settings` | None | New: persist to DiskCache/JSON config |
| `GET /api/logs` | `API.get_logs()` | Wrap existing LogBuffer |
| `WebSocket /ws` | `_poll_loop()` | Replace polling with async event push |
...
### Automated Tests
```bash
# After backend implements the API:
...
### Manual Verification
1. Start the FastAPI server: `python -m api_server`
2. Hit `GET /api/health` with curl — verify 200 response
3. Hit `POST /api/session/start` with a watchlist — verify adapters launch
4. Open WebSocket connection — verify events stream
5. Hit `GET /api/positions` — verify enriched position data
6. Upload an algorithm via `POST /api/algorithms` — verify file saved
...
> [!IMPORTANT]
> **Q1**: Should the API support multiple simultaneous strategies per session, or only one strategy at a time? The current architecture (one `SIM_SYMBOL` per Handler.py process) suggests one-at-a-time, but the UI has an "Active Algos" counter implying multiple.
>
> **Recommendation**: Support multiple strategies running simultaneously, each as a separate subprocess with its own ZMQ DEALER identity. The "Active Algos" counter reflects the count of running strategy subprocesses.

> [!IMPORTANT]  
> **Q2**: Should settings be persisted in a JSON file (`~/.paperquant/settings.json`) or in the DiskCache? JSON file is human-readable and easy to backup; DiskCache is already in the stack.
>
> **Recommendation**: JSON file at `~/.paperquant/settings.json` for portability and user-editability.

> [!NOTE]
> **Q3**: The current `order_history.csv` uses a flat CSV format. Should we migrate to SQLite for trade history (better querying, atomicity)?
>
> **Recommendation**: Yes, add a `trades` table to the existing `paperquant.db` SQLite database.
