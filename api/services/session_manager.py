...
class SessionManager:
    def __init__(self, base_dir: str | Path | None = None):
        ...
        self.portfolio = PortfolioService(
            self.state_cache_path, self.live_prices_cache_path
        )
    ...
    async def start_session(
        ...
        with Cache(self.state_cache_path) as state_cache:
            for item in watchlist:
                ticker = item["ticker"].upper()
                capital = float(item.get("capital", 10000.0))
                state_cache.set(f"capital:{ticker}", capital)
    ...
    async def reset_session(self) -> dict:
        ...
        for path in (
            ...
            self.live_prices_cache_path,
            self.temp_dir / "cache_candles",
        ):
        ...
        result = {
            **self.active_session,
            ...
            "strategy": {
                ...
                "name": strategy_name,
                "status": "running"
                if self.active_session.get("strategy_id")
                else "stopped",
            },
            "adapters": {
                "price_adapter": self.process_manager.get_status("price_adapter"),
        ...
        return [
            {
                ...
                "symbol": r.get("Symbol", ""),
                "side": r.get("Action", ""),
                "qty": int(float(r.get("Executed_Quantity", 0))),
                "price": float(r.get("Executed_Price", 0)),
                "order_type": "market",
            }
            for r in rows
        ]
        ...
        while True:
            try:
                ...
                stats = self.portfolio.get_aggregate_stats(
                    ...
                    sum(
                        1
                        for name in self.process_manager.processes
    ...
    async def _broadcast_adapter_status(self, force: bool = False) -> None:
        ...
        for adapter, status in current.items():
            if force or self._last_adapter_status.get(adapter) != status:
                await manager.broadcast(
                    "adapter_status", {"adapter": adapter, "status": status}
                )
