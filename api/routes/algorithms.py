...
async def list_algorithms(request: Request):
    return {"algorithms": request.app.state.session_manager.algorithm_store.list_algorithms()}


@router.post("")
async def register(request: Request, file: UploadFile = File(...), name: str = Form(...),
                   dependencies: str = Form("")):
    manager = request.app.state.session_manager
    if not file.filename or not file.filename.endswith(".py"):
        return JSONResponse(status_code=400, content={"error": "file_invalid", "message": "Upload a Python script."})
    try:
        ...
        return JSONResponse(status_code=400, content={"error": "file_invalid", "message": "Invalid Python script filename."})
