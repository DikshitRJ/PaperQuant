...
def supertrend(symbol, period, interval, multiplier=3):
    ...
    for i in range(1, len(high_ser)):
        ...
        # Final Upper Band
        if (
            basic_upperband[i] < final_upperband[i - 1]
            or close_ser[i - 1] > final_upperband[i - 1]
        ):
        ...
        # Final Lower Band
        if (
            basic_lowerband[i] > final_lowerband[i - 1]
            or close_ser[i - 1] < final_lowerband[i - 1]
        ):
        ...
        # Supertrend
        if (
            supertrend_vals[i - 1] == final_upperband[i - 1]
            and close_ser[i] <= final_upperband[i]
        ):
            supertrend_vals[i] = final_upperband[i]
        elif (
            supertrend_vals[i - 1] == final_upperband[i - 1]
            and close_ser[i] > final_upperband[i]
            or supertrend_vals[i - 1] == final_lowerband[i - 1]
            and close_ser[i] >= final_lowerband[i]
        ):
            supertrend_vals[i] = final_lowerband[i]
        elif (
            supertrend_vals[i - 1] == final_lowerband[i - 1]
            and close_ser[i] < final_lowerband[i]
        ):
            supertrend_vals[i] = final_upperband[i]
        else:
