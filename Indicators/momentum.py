...
def rsi(symbol, period, interval):
    ...
    close = pd.Series(
        candle_list(symbol, period * 3, interval, field="close"), dtype="float64"
    )
    if close.empty:
        return None
...
def stochastic_oscillator(
    symbol, period, interval, k_period=14, smooth_k=3, smooth_d=3
):
    """
    ...
    return {"k": percent_k_smoothed.iloc[-1], "d": percent_d.iloc[-1]}


def stochastic_rsi(
    symbol, period, interval, rsi_period=14, stoch_period=14, smooth_k=3, smooth_d=3
):
    """
    ...
    candles = candle_list(
        symbol, (rsi_period * 3) + stoch_period + 10, interval, field="close"
    )
    if not candles:
        return None
    ...
    return {"k": percent_k.iloc[-1], "d": percent_d.iloc[-1]}
...
def roc(symbol, period, interval, roc_period=12):
    ...
    """
    close = pd.Series(
        candle_list(symbol, period + roc_period + 5, interval, field="close"),
        dtype="float64",
    )
    if close.empty:
        return None
...
def tsi(symbol, period, interval, long=25, short=13):
    ...
    """
    close = pd.Series(
        candle_list(symbol, period + long + short + 20, interval, field="close"),
        dtype="float64",
    )
    if close.empty:
        return None
...
def ppo(symbol, period, interval, fast=12, slow=26, signal=9):
    ...
    """
    close = pd.Series(
        candle_list(symbol, period + (slow * 3), interval, field="close"),
        dtype="float64",
    )
    if close.empty:
        return None
